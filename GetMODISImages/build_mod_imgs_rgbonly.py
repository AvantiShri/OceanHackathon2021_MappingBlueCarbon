...
def subset_site_data(csv, prod):
    ...
    #for index, row in coordinates.iterrows():
        ...
        for doi in time_idx[row['site_tag']]:
            ...
            data_obj['name'] = os.path.join(fdir, prod+'_'+str(row['latitude'])+'_'+str(row['longitude'])+'_'+doi+'_rgb.png')
            #if data_obj['name'] doesn't exist yet, do call
            if os.path.exists(data_obj['name']):
                print('file exists, skipping',data_obj['name'])
            else:
                for band in BANDS:
